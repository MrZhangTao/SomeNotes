...
现在计算机系统通用的字符编码工作方式是：
    在计算机内存中，统一使用Unicode编码，当需要保存到硬盘或者需要传输的时候，就转换为UTF-8编码。[更换编码方式以节省内存]
...
如果客户端版本低于服务端版本，则下载最新的资源，并将客户端的资源版本更新至服务端的版本。
由于我们没有对打包出来的APK中的资源的写权限，所以我们需要将下载下来的资源放在一个可以读写的路径中，
...
34.在web开发中，“route”是指根据url分配到(跳转执行)对应的处理程序
...
37.python支持连续比较：print("a" < "b" < "c") 返回 True，相当于"a" < "b" and "b" < "c"

38. python2中除法默认向下取整，因此1/2=0，为整型；
    python3中除法为正常除法，会保留小数点，为浮点型(使用底板除 // 则会向下取整)

39.python的标准数据类型为：
    Number(数字) 细分为 int long float complex(复数)
    String(字符串)(没有字符类型char)
    List(列表)(可变类型)
    Tuple(元组)
    Dictionary(字典)(可变类型)


40.python中 __new__和__init__：
    __new__ 是构造函数，静态函数，会返回一个对象，先于后者执行
    __init__是初始化函数，实例函数，对象被构造之后，再来初始化

41.Python中，functools.partial的作用是把一个函数的某些参数给固定住(设定默认值),
            返回一个新的函数，使得调用函数的时候，能达到自己想要的便捷
